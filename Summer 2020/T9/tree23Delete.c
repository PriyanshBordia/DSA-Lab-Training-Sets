...
newRight = NULL;
insert2(toRoot, data);
/* check if tree height needs increase */ if (newRight != NULL) {
toRoot = mkNode(newKey); toRoot->subT[0] = oldRoot; toRoot->subT[1] = newRight;
} }
static void insert2(node23p node, int data) { assert(node != NULL);
int kys, temp;
// Primarily to insert in a LEAF
if (node->subT[0] == NULL) {// This is a leaf
if (node->data[0] == data) return; // Duplicate data
if (node->countKeys == 2) { // Full leaf node if (node->data[1] == data)
return; // Duplication of data
// Tell parent to care for new right sibling. splitLeaf(node, data);
return;
}
// Add data in this leaf node node->data[node->countKeys] = data; node->countKeys += 1;
kys = node->countKeys-1;
while (kys>0) {
if (node->data[kys-1] <= node->data[kys]) return; // Node set up
// Swap

temp = node->data[kys-1]; node->data[kys-1] = node->data[kys]; node->data[kys] = temp;
return; }
// Insertion into an internal node
assert(node->subT[0] != NULL && node->subT[1] != NULL); assert(node->countKeys == 1 || node->subT[2] != NULL); /* When inserting data at an internal node there are
two phases of importance. So we separately code it */ insertInternal(node, data);
static void splitLeaf(node23p node, int data) {
// Use newRight to tell the parent to update. assert(node->countKeys == 2);
int temp = node->data[1];
node->countKeys -= 1; if (temp < data) {
newRight = mkNode(data); newKey = temp;
return;
}
newRight = mkNode(node->data[1]); if (node->data[0] < data) {
newKey = data;
return; }
newKey = node->data[0]; node->data[0] = data; return;
}
