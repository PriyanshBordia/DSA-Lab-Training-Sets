...
#define PRIMARY_HASHING_METHOD DIVISION
...
int goldenMultiplicative(int key, int size) 
{
	/* Returns the primary location for the data
		...
		The location is computed using multiplication by golden number. Reference: Reema Thareja Edn 2 page 468
	*/
    ...
    switch (PRIMARY_HASHING_METHOD) 
    {
    	case DIVISION:
			switch (PROBING_METHOD) 
			{
				...
				case QUADRATIC_REHASH:
					return (key + STEP * probe * probe) % usedSize;
		...
		case MULTIPLICATIVE:
			...
			switch (PROBING_METHOD) 
			{
				...
				case QUADRATIC_REHASH:
					return (goldIndx + STEP * probe * probe) % usedSize;
...
int main(void) 
{
	...
	*/
	...
	while (!feof(keys))
	{
		...
		dataP = malloc(sizeof(strlen(strg) + 1));
